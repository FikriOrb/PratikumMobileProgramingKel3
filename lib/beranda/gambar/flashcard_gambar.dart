...
import 'flashcard_gambar_view.dart';
import 'flashcard_gambar_edit.dart';

class FlashcardGambarPage extends StatefulWidget {
  const FlashcardGambarPage({super.key});

  @override
  State<FlashcardGambarPage> createState() => _FlashcardGambarPageState();
}

class _FlashcardGambarPageState extends State<FlashcardGambarPage> {
      ...
      builder: (context) => Dialog(
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              Text(isEdit ? "Ganti Nama Album" : "Buat Album Baru", style: GoogleFonts.lexend(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.white)),
                  ...
                  ElevatedButton(
                    ...
                    onPressed: () {
                      if (controller.text.isNotEmpty) {
                        if (isEdit) {
                          context.read<UserData>().editImageAlbumTitle(id, controller.text);
                        } else {
                          context.read<UserData>().addImageAlbum(controller.text);
                        }
  ...
  void _showDeleteConfirm(BuildContext context, String id, String title) {
    showDialog(context: context, builder: (context) => AlertDialog(backgroundColor: bgCard, title: Text("Hapus Album?", style: GoogleFonts.lexend(color: Colors.white)), content: Text("Hapus '$title'?", style: TextStyle(color: textGray)), actions: [TextButton(onPressed: () => Navigator.pop(context), child: Text("Batal", style: TextStyle(color: textGray))), TextButton(onPressed: () { context.read<UserData>().deleteImageAlbum(id); Navigator.pop(context); }, child: const Text("Hapus", style: TextStyle(color: Colors.redAccent)))]));
  }

  void _showOptionsDialog(BuildContext context, String id, String title, List<Map<String, String>> items) {
    showDialog(context: context, builder: (context) { return Dialog(backgroundColor: Colors.transparent, insetPadding: const EdgeInsets.all(20), child: Container(padding: const EdgeInsets.all(24), decoration: BoxDecoration(color: bgCard, borderRadius: BorderRadius.circular(24), border: Border.all(color: Colors.white10), boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.5), blurRadius: 20, offset: const Offset(0, 10))]), child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [Center(child: Text("Pilih Aksi", style: GoogleFonts.lexend(fontSize: 14, color: textGray, fontWeight: FontWeight.w500))), const SizedBox(height: 24), 
      ...
      _buildOptionTile(
        icon: Icons.play_circle_fill, iconBgColor: primaryPurple, iconColor: Colors.white, 
        title: "Lihat Kartu", subtitle: "Mode belajar gambar", 
        onTap: () { 
          Navigator.pop(context); 
          if (items.isEmpty) { 
            ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Album ini masih kosong!"))); return; 
          } 
          Navigator.push(context, MaterialPageRoute(builder: (context) => FlashcardGambarViewPage(albumId: id))); // KIRIM ID
        }
      ), 
      ...
      _buildOptionTile(
        icon: Icons.image_search, iconBgColor: Colors.white10, iconColor: Colors.white, 
        title: "Kelola Gambar", subtitle: "Tambah/Hapus gambar", 
        onTap: () { 
          Navigator.pop(context); 
          Navigator.push(context, MaterialPageRoute(builder: (context) => FlashcardGambarEditPage(albumId: id))); // KIRIM ID
        }
      ), 
      ...
      body: Stack(children: [
          ...
          SafeArea(
            child: Column(children: [
              Padding(padding: const EdgeInsets.all(16.0), child: Row(children: [InkWell(onTap: () => Navigator.pop(context), borderRadius: BorderRadius.circular(20), child: Container(width: 40, height: 40, decoration: BoxDecoration(color: Colors.white.withOpacity(0.1), shape: BoxShape.circle), child: const Icon(Icons.arrow_back, color: Colors.white))), const SizedBox(width: 16), Expanded(child: Text("Flash Card: Gambar", style: GoogleFonts.lexend(fontSize: 20, fontWeight: FontWeight.bold, color: Colors.white))), InkWell(onTap: () => _showSkinSelector(context), borderRadius: BorderRadius.circular(20), child: Container(width: 40, height: 40, decoration: BoxDecoration(color: Colors.white10, shape: BoxShape.circle), child: const Icon(Icons.palette, color: Colors.pinkAccent))), const SizedBox(width: 12), InkWell(onTap: () => _showFormDialog(context), borderRadius: BorderRadius.circular(20), child: Container(width: 40, height: 40, decoration: BoxDecoration(color: primaryPurple, shape: BoxShape.circle), child: const Icon(Icons.add, color: Colors.white)))])) , 
              
              // CONSUMER USER DATA
              Expanded(
                child: Consumer<UserData>(
                  builder: (context, userData, child) {
                    var albums = userData.imageAlbums;
                    if (albums.isEmpty) return Center(child: Text("Belum ada album.", style: TextStyle(color: textGray)));
                    
                    return ReorderableListView.builder(
                      padding: const EdgeInsets.symmetric(horizontal: 16), 
                      onReorder: (oldIndex, newIndex) => userData.reorderImageAlbums(oldIndex, newIndex), 
                      ...
                      itemBuilder: (context, index) { 
                        ...
                        return _buildListItem(context, index, album['id'], album['title'], items); 
                      }
                    );
                  }
                )
  ...
  Widget _buildListItem(BuildContext context, int index, String id, String title, List<Map<String, String>> items) {
    int jumlah = items.length;
    return Container(
      ...
      child: Row(
        children: [
          ...
          Container(padding: const EdgeInsets.all(8), decoration: BoxDecoration(color: primaryPurple.withOpacity(0.1), shape: BoxShape.circle), child: Icon(Icons.image, color: primaryPurple, size: 18)),
          const SizedBox(width: 12),
          
          Expanded(
            child: InkWell(
              onTap: () => _showOptionsDialog(context, id, title, items),
              child: Padding(
                padding: const EdgeInsets.symmetric(vertical: 16),
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(title, style: GoogleFonts.lexend(fontSize: 16, fontWeight: FontWeight.w500, color: Colors.white)), Text("$jumlah Gambar", style: GoogleFonts.lexend(fontSize: 12, color: textGray))]),
              ),
            ),
          ),
